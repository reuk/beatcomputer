...
#define R_INSTRUCTION_IMPLEMENTATION(Name, String, Id, functionbody, tooltip) \
    void Name::execute_specific(Core &core,                                   \
...
BINARY_INSTRUCTION_IMPLEMENTATION(AND,
                                  0x0B,
                                  &&,
                                  "AND    a b c   -> a = b && c");
BINARY_INSTRUCTION_IMPLEMENTATION(OR, 0x0C, ||, "OR     a b c   -> a = b || c");

R_INSTRUCTION_IMPLEMENTATION(
    OSC_R,
    "OSC",
    0x0F,
    send_osc(osc_out_port, osc_out_prefix, osc_out_address, rd, rs, rt),
    "OSC    a b c   -> send osc");
I_INSTRUCTION_IMPLEMENTATION(
    OSC_I,
    "OSC",
    0x0F,
    send_osc(osc_out_port, osc_out_prefix, osc_out_address, rt, rs, im),
    "OSC    a b c   -> send osc");
R_INSTRUCTION_IMPLEMENTATION(
    JUMP_R, "JUMP", 0x12, core.ip = rd - 1, "JUMP   a       -> go to a");
I_INSTRUCTION_IMPLEMENTATION(
    JUMP_I, "JUMP", 0x12, core.ip = im - 1, "JUMP   a       -> go to a");
I_INSTRUCTION_IMPLEMENTATION(JE,
                             "JE",
                             0x13,
                             if (rs == rt) core.ip = im,
                             "JE     a b c   -> if a == b go to c");
I_INSTRUCTION_IMPLEMENTATION(JNE,
                             "JNE",
                             0x14,
                             if (rs != rt) core.ip = im,
                             "JNE    a b c   -> if a != b go to c");
I_INSTRUCTION_IMPLEMENTATION(JL,
                             "JL",
                             0x15,
                             if (rs < rt) core.ip = im,
                             "JL     a b c   -> if a < b go to c");
I_INSTRUCTION_IMPLEMENTATION(JLE,
                             "JLE",
                             0x16,
                             if (rs <= rt) core.ip = im,
                             "JLE    a b c   -> if a <= b go to c");
I_INSTRUCTION_IMPLEMENTATION(JG,
                             "JG",
                             0x17,
                             if (rs > rt) core.ip = im,
                             "JG     a b c   -> if a > b go to c");
I_INSTRUCTION_IMPLEMENTATION(JGE,
                             "JGE",
                             0x18,
                             if (rs >= rt) core.ip = im,
                             "JGE    a b c   -> if a >= b go to c");

I_INSTRUCTION_IMPLEMENTATION(LOAD,
                             "LOAD",
                             0x10,
                             rt = memory[rs + im].raw,
                             "LOAD   a b c   -> a = memory[b + c]");
I_INSTRUCTION_IMPLEMENTATION(SAVE,
                             "SAVE",
                             0x11,
                             memory[rs + im].raw = rt,
...
R_INSTRUCTION_IMPLEMENTATION(
    BNOT_R, "BNOT", 0x07, rd = ~rs, "BNOT   a b     -> a = ~b");
I_INSTRUCTION_IMPLEMENTATION(
    BNOT_I, "BNOT", 0x07, rt = ~im, "BNOT   a b     -> a = ~b");
R_INSTRUCTION_IMPLEMENTATION(
    NOT_R, "NOT", 0x0D, rd = !rs, "NOT    a b     -> a = !b");
I_INSTRUCTION_IMPLEMENTATION(
    NOT_I, "NOT", 0x0D, rt = !im, "NOT    a b     -> a = !b");
R_INSTRUCTION_IMPLEMENTATION(
    MOVE_R, "MOVE", 0x0E, rd = rs, "MOVE   a b     -> a = b");
I_INSTRUCTION_IMPLEMENTATION(
    MOVE_I, "MOVE", 0x0E, rt = im, "MOVE   a b     -> a = b");
R_INSTRUCTION_IMPLEMENTATION(
    RND, "RND", 0x19, rd = rand(), "RND    a       -> a = rand()");
